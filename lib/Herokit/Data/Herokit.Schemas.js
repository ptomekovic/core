...
Herokit.Data.setSchema = function(name, data, collectionName){
    ...
    //console.log(Herokit.Data._schemas[name]);
};
Herokit.Data.appendSchema = function(name, data, collectionName){
    ...
    if (!(BaseSchema instanceof SimpleSchema)) BaseSchema=new SimpleSchema(BaseSchema);

    if (data instanceof SimpleSchema)
        var appendSchema = data;
    else
        var appendSchema = new SimpleSchema(data);

    var newSchema=Herokit.Data.mergeSchemas({BaseSchema: BaseSchema, newSchema: appendSchema});
    newSchema = newSchema._schema;
...
Herokit.Data.getFieldRoles = function(collection, field){
    var schemaData;
    if (Herokit.Helper.debug.isLog(5)) console.log("Herokit.Data.getFieldLabel ", collection, field);
    ...
    if (typeof schemaData[field].roles !== 'undefined') return schemaData[field].roles;
    else return [];
